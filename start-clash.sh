...
create_iptables_rules() {
  if ! /usr/sbin/iptables -L -n -t nat | grep -lq CLASH ; then
    ...
    /usr/sbin/iptables -t nat -A PREROUTING -p udp -m udp --dport 53 -j DNAT --to-destination 192.168.11.1:12358
  fi
}

start_clash() {
  killall clash-linux-armv8
  nohup /mnt/ASUS/clash-linux-armv8 -d /mnt/ASUS/ > /mnt/ASUS/nohup.out &
}
